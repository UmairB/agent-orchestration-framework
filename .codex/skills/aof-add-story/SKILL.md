...
Use this skill when the user asks for `$aof-add-story <story description> [under milestone NN]`, or asks to run the AOF `aof:add-story` procedure in Codex.

Where this procedure mentions `$ARGUMENTS`, use the text the user supplied after the skill name.
Where it mentions Claude slash command `/aof:add-story`, treat that as this Codex skill invocation.
