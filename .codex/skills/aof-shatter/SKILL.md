...
Use this skill when the user asks for `$aof-shatter [PRD path - omit to auto-discover PRD-*.md]`, or asks to run the AOF `aof:shatter` procedure in Codex.

Where this procedure mentions `$ARGUMENTS`, use the text the user supplied after the skill name.
Where it mentions Claude slash command `/aof:shatter`, treat that as this Codex skill invocation.
