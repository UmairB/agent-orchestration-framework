...
Use this skill when the user asks for `$aof-add-uat <short session description> [accepting NN[,NN…]]   (depends optional - defaults to the delivered span)`, or asks to run the AOF `aof:add-uat` procedure in Codex.

Where this procedure mentions `$ARGUMENTS`, use the text the user supplied after the skill name.
Where it mentions Claude slash command `/aof:add-uat`, treat that as this Codex skill invocation.
