...
- [ ] **Fitness `acd-claim-relay-independent`** (arch-test, ADR-004 / fitness #9, the 23/fitness-#4 mirror) — in `commands/run-start.mjs` the local claim write + git sync are NOT nested in any relay-push conditional; the `pushLeaseSignal` call sits inside `try`/`catch`.
- [ ] **Fitness `acd-relay-lease-blind`** (arch-test, ADR-004 / fitness #10) — `src/mesh-relay.mjs` contains NO lease reference (no lease-kind literal, no lease-module import); `LEASE_SIGNAL_KIND` lives in `mesh-relay-client.mjs` only. GREEN now — must STAY green (the gate exists to catch the tempting edit).
- [ ] **Fitness `acd-lease-cache-only`** (arch-test, ADR-004 / fitness #11) — the lease apply branch + lease cache perform no durable write and import no persist seam; extends the existing `acd-presence-subscriber-cache-only` coverage with the m03 non-vacuous planted-violation self-check.
- [ ] **Fitness `acd-fleet-reclaim-guarded`** (arch-test, ADR-006 / fitness #12) — the fleet item set is built ONLY inside the mesh-configured branch and filtered through `isNodeStale` BEFORE `reclaimStaleRuns`; the store's signature/mesh-blindness unchanged; rollback still routes through `rollbackItemStatus`; the existing `acd-run-reclaim-stale-only` + `acd-status-rollback-bounded` re-arm green.
