...
- **⛔→✅ SOAK-BLOCKER F21 + F22 CLOSED `2026-07-24` via `aof:continue 38/04` — and the review found THREE more defects underneath them, each of a DIFFERENT class. Story 04 back to `in-review`, soak owed again.** The `2026-07-24` live two-machine soak (task 04) was the first of this milestone to actually run, and it FAILED: the operator clicked let-shield's `ref 18` and the mesh minted an assignment for the CONTROL's `ref 18` — a different milestone entirely — off a `200 ok`. Re-opened, decided, built, three-lens reviewed.
  - **F21 (BLOCKER) — the route now targets the ITEM's workspace.** ADR-012 AMENDMENT: the wire is `{ref,nodeId,workspaceId}`, **all REQUIRED** (a missing id is a coded refusal, never a fallback — the fallback WAS the defect); resolution runs the sanctioned `queryGlobalMeshStatus → status.workspaces[] → projectRoot → loadWorkspace` seam (the `board-url` precedent, **zero new imports**, so inv.4's pinned import list stays byte-identical); the mint's target is asserted **PRE-mint** with the verb-identical derivation; four coded refusals (`invalid-workspace` 400 / `workspace-not-found` 404 / `workspace-not-local` 409 / `workspace-id-mismatch` 409). New armed fitness **`acd-fleet-assign-targets-item-workspace`**, independently measured by the orchestration at **10 violations against HEAD → 0 after the fix**.
  - **F22 — the affordance now acknowledges.** DESIGN §Surface 2 Amendment: a 2xx puts the SAME button into a `muted`, disabled **`Sent`** with the picker frozen on the chosen node, held exactly one poll interval then decayed to rest; plus **ONE** silent keep-last-good re-load so region 5's m35 chip lands within a round trip. `Sent` reports the CALL, never the assignment's STATE — a dispatch can be sent and then fail (the soak's own +1.5s).
  - **⛔→✅ F-38.04a / GAP-S2-3 (QA, HIGH) — F21's defect class RELOCATED TO THE NODE AXIS, found in a real browser.** `AssignAffordance` seeded `useState(options[0])` and **never re-synced when the roster changed**. On a monitor polling every 5s, when the selected node leaves the roster the `<select>`'s DOM value coerces to the first survivor while React state keeps the stale id — so the row displayed one target and POSTed another (`Sent` beside `zzz-second-node`, chip reading `→ aaa-firs…`, store row `aaa-first-node`). The trigger is this codebase's own asymmetry: `queryGlobalRegistry` drops a node whose descriptor file does not resolve, while `assignWork`'s node-known gate reads `global_nodes` DIRECTLY — so a node can leave the picker and stay assign-eligible. Fixed by **deriving rather than remembering** the target. **Lesson: A7's "the target is named for free" makes a stale selection WORSE — an acknowledgment that names the wrong target is more dangerous than none.**
  - **⛔→✅ F-B (architect) — `issuer` rode off the TARGET workspace.** The amendment reasoned about the one fact the verb derives from the workspace object it is handed (`workspaceId`) and missed the **second**, three lines below: `issuer = ctx.issuer ?? workspace.config?.mesh?.nodeId` — a **machine**-scoped fact read off the target. Benign on the common path; on a machine with a legacy per-workspace sidecar it stamps the wrong issuer, and with no identity at all it is `null` against `NOT NULL` — an uncoded 500. Not decoration: a revoked issuer's directive never routes. Fixed by passing the control's own identity explicitly, plus a named `control-identity-unknown` refusal. **Lesson (the durable one): when you change WHICH OBJECT a verb is handed, you change EVERY fact the verb derives from it — enumerate them all; asserting one derived field is not asserting the object.**
  - **⛔→✅ DG-13 + DG-14 (designer, from the REAL render) — §Surface 2's FIRST REAL VERDICT after six days: GAPS at 1280.** The render this checklist had been owed since `2026-07-18` was finally produced — twelve frames, real built `ui/dist` (post-DG-12), real route, real minted record, **nothing hand-seeded**. It found (a) the picker collapsing to a **bare chevron** under an inline error, so the target is anonymous at the exact moment the operator must re-aim, and the row reflowing on every label swap; and (b) a hung POST wedging `Assigning…` **forever**, a state the design had never described. Both decided as DESIGN rules (fixed action width, a ≥14ch picker floor, message-yields, copy priority *outcome > holder*, region-5 target-truncates-last; and a 10s = 2×`POLL_MS` timeout landing in `refused` reading `no answer — timed out`) and built as tasks 06/07.
  - **The methodological finding, and the reason this pass matters beyond its bugs: ADR-008's producer-fed rule was HONOURED on story 04 and still missed F21.** Tasks 00–03 drive the REAL route, the REAL verb, and read every mint back from the REAL store — honest producer-fed lanes. They were blind because **producer-fed constrains the DATA, not the CONFIGURATION**: a single-workspace fixture cannot express a wrong-target defect, since the caller's own workspace is the only candidate and the right answer and the wrong answer are the same value. **Addable clause to ADR-008: a seam whose defect is a WRONG TARGET must be exercised with ≥2 candidate targets, one of which is not the caller's own — and, where the discriminator is an identifier, with that identifier COLLIDING across them.** The soak's `ref 18` existing in both workspaces is exactly what turned a silent failure into a silent wrong dispatch. Both new fixtures instantiate that clause; **nothing yet stops a fifth seam from shipping without it** (this was the THIRD Gap-A recurrence, and F-38.04a was the fourth at the node axis).
  - **Final gate:** story-04 focused surface **68 ok / 0 not ok** and the RED→GREEN fitness measurement both re-verified **by the orchestration itself**, not taken from the sub-agents' reports; blast radius **225 ok / 0** over 35 mesh-ui/fleet suites; `npm run ui:build` green. **Story 04 stays `in-review`** — task 04's `@manual` soak is owed AGAIN on the fixed build, and a §Surface 2 **re-render is owed** (DG-13/DG-14 changed the row's geometry and added a state; GAP-S2-3's fix is confirmed by a re-run frame but the surface verdict stays **GAPS** until re-judged). **Milestone stays `in-progress`.**
  - **⚠️ Routed OUT (pre-existing, flagged twice this pass):** `test/mesh-ui-write-isolation-bounded.test.mjs` carries 3 permanently-red lanes against the m27 `POST /api/mesh/issue` route retired at m33, and is **not registered** in `scripts/test.mjs` — invisible to the suite. Retire the file; route with the four stale `mesh-sync.mjs` arch tests to the stabilisation chore.
  - **Latent infra bug fixed in passing:** `test/support/mini-react.mjs`'s fake clock swallowed Node-internal `clearTimeout` (it returned early for any token without its own `.id`), so undici's per-request timeout was never cleared and an orphan timer later killed whole runs with a destroyed-parser `TypeError`. Measured: pre-fix 2-of-3 blast-radius runs crashed; post-fix 0-of-7.
...
- **Refine `2026-07-18` (`aof:refine 38 --autonomous`) — stories 03–08 fully contracted; documented default decisions
  ...
  - **04 (ADR-012, AMENDED `2026-07-24`):** endpoint `POST /api/mesh/assign` **`{ref,nodeId,workspaceId}` — all three
    REQUIRED** (was `{ref,nodeId}`; the amendment closes BLOCKER F21 — a missing `workspaceId` is a coded refusal, NEVER
    a fallback to the daemon's own workspace, because that fallback *was* the defect). The route resolves the **item's
    OWN** workspace through the sanctioned seam (`queryGlobalMeshStatus` → `status.workspaces[]` → `projectRoot` →
    `loadWorkspace`) and asserts the resolved workspace's own id equals the requested one **before** minting. Four
    route-level refusal codes, none of them a fallback: `invalid-workspace` 400 · `workspace-not-found` 404 ·
    `workspace-not-local` 409 · `workspace-id-mismatch` 409 (the *verb's* `ASSIGN_GATE_STATUS` mapping is untouched).
    Admission = **loopback-bound + same-origin local-admission, no auth token this story** — a networked multi-operator
    fleet face would need a real auth gate (explicitly out of scope). The exact 4xx numbers for the *verb's* gate misses
    remain a build mapping (the contract pins the *code*, not the number). ADR-012 grounded on the seam's history:
    m27/ADR-006 shipped a fleet-face write route later RETIRED, m35/ADR-007 deferred UI-assign — this is the third pass
    (carry to retro).
...
- **⛔ SOAK-BLOCKER F21 (`aof:verify 38` soak 04, `2026-07-24`; built out at `aof:continue 38/04`) — the ADR-010 "Gap A" class for the THIRD time in ONE milestone, and the first defect here that dispatched WRONG WORK off a `200 ok`.** A **global, machine-wide surface resolved a PER-ITEM fact from its own local context**: the fleet face lists items from every workspace on the machine, but `POST /api/mesh/assign` carried only `{ ref, nodeId }` and resolved the ref against `resolvedProjectDir` — the daemon's OWN project dir. The operator clicked let-shield's `ref 18` and the mesh minted an assignment for the CONTROL's `ref 18`, a completely different milestone. Distilled lessons (carry forward / graduate to ADRs):
  - **The durable rule, now stated three times in three stories: a global surface may NEVER resolve a per-item fact from its own local context.** `cloneUrl` (story 02, ADR-010 Gap A) → App identity (story 03, ADR-011) → the assign target (story 04, F21). Each was fixed in isolation as "this seam's bug"; none of the three fixes made the NEXT one impossible. The class deserves a milestone-level ADR or a fitness template, not a fourth per-seam patch.
  - **⛔⛔ ADR-008's producer-fed rule was HONOURED here and still missed it — because producer-fed constrains the DATA, not the CONFIGURATION.** Story 04's tasks 00–03 drive the REAL route and the REAL verb and read every mint back from the REAL store; they are honest producer-fed lanes. They still could not see F21, because a **single-workspace fixture cannot express the failure** — the server's own workspace is the only workspace there is, so the wrong answer and the right answer are the same value. **Addable clause to ADR-008: a seam whose defect is a WRONG TARGET must be exercised in a fixture carrying at least TWO candidate targets, one of which is not the caller's own — and, where the discriminator is an identifier, with that identifier COLLIDING across them.** The soak's ref `18` existed in both workspaces; that collision is what turned a silent failure into a silent wrong dispatch.
  - **A `200 ok` on the wrong target is a worse failure mode than a 500 on the right one, and the design must be read that way.** The mis-dispatch was invisible at every layer that reported success: the route returned 200, the record minted cleanly, the store was consistent. Only the *human* knew the milestone title was wrong. On a worker holding the control's repo this would have executed real, unintended work. **When a route's failure mode is "correct-looking success", the refusal path is the feature** — hence the amendment's four coded refusals and its pre-mint identity assertion (verify the thing you are about to write ALREADY equals what the caller asked for, rather than trusting the resolution that produced it).
  - **⚙️ ACD TOOLING GAP (found `2026-07-24`, affects every milestone): `aof-designer` cannot amend a mature
    record doc — it has `Write` but no `Edit`.** Asked to record its §Surface 2 verdict, the designer had to
    decline the write: `DESIGN.md` is 1277 lines and a whole-file `Write` does not fit in one output, so it
    correctly refused rather than risk a **truncated clobber of the milestone's binding conformance baseline**,
    and returned nine dictated edits for the orchestration to apply by hand. It made the right call — but the
    failure mode it avoided is the same class as the "evidence subagents can clobber records" lesson, arriving
    through the tool list instead of through carelessness. **Every doc-owning agent's capability must match the
    doc's maturity: an agent that owns a record doc for its whole lifetime needs `Edit`, because record docs only
    grow.** Until fixed, a designer pass on any mature DESIGN.md silently costs a full hand-transcription round.
  - **Design note (F22, same soak): a silent success is a failure of the surface.** §Surface 2 A7 as authored deliberately specified "on success, no local chip — the confirmation surfaces via the existing 5s poll." That was a defensible one-ramp decision on paper and it did not survive one real operator: on a monitor that repaints itself every 5s, "nothing changed" is indistinguishable from "nothing happened", and the operator fell back to reading the raw API response. **A mutation affordance on a passively-refreshing surface owes an acknowledgment in the region the hand is already in, within the interaction's own time horizon — not merely eventual consistency in a quieter region.**
...
- [x] Story-04 **ACCEPTED** (`aof:verify 38/04`, `2026-07-24`) — the decline below was superseded IN THE SAME PASS: the operator directed the design gaps be fixed rather than routed to another `aof:continue`, so they were built and the surface re-judged **four times** until it converged (GAPS → GAPS → GAPS-non-blocking → residues closed). Closed this pass: **DG-15/16/17** (overprint, `l…` stub, truncated holder), **DG-19/20/21** (squeeze-where-a-drop-is-needed + the row leaving the card's box, chip-gate→fit-gate, a ladder rung with no outcome word), **DG-22** (cluster drift). Deferred: **DG-18**, **DG-23** (predates story 04). Final: story lanes **70/0** (a lane ADDED pinning the tail-drop), blast radius **227/0**, arch sweep **694/703** (same 9 pre-existing reds), `ui:build` green, `validate` PASS. **The build lesson, worth the retro:** the yield order was first written as flex `shrink` factors (1000000:1000:1) and MEASURED to do the opposite of what the ratio implies — the drill-in yielded 13.1px while the chip, weighted 1, yielded 17.5px. **Flexbox distributes a squeeze; it cannot express "this element goes away so that one can be whole."** Every clause is now a discrete budgeted drop. Two further defects (`min-w-0` letting the pinned arrow escape the card's box vs. `min-width:auto` resolving to full content width so it never yielded; `flex-1` making a KEPT workspace name GROW and squeeze the target) were invisible in the markup and came off a `getBoundingClientRect` ledger. **Task 04's `@manual` soak is NOT part of this accept** — it is the milestone's deferred human gate (the story-02 precedent), still owed at `aof:verify 38` on a redeployed SEA. **Milestone stays `in-progress`**
- [x] ~~Story-04 verify pass (`aof:verify 38/04`, `2026-07-24`, post-F21/F22 build) — **DECLINED, stays `in-review`.**~~ (superseded above) Automated lanes green and re-measured honestly: story-04 focused surface **68/0** (real case-array runner, per-case isolated `AOF_GLOBAL_HOME`), blast radius **225/0** over 34 mesh-ui/fleet/assignment suites, full arch sweep **694 ok / 9 not ok** (all 9 the known pre-existing `mesh-sync.mjs`/m41 reds — **no m38 fitness red**), `ui:build` green. §Surface 2 **re-render produced and judged: GAPS at 1280** — GAP-S2-2/DG-14 and GAP-S2-3 **CLOSED**, GAP-S2-1/DG-13 closed as filed, but **three successors**: **DG-15** (a long target OVERPRINTS `Open board →` — build defect, a paint order where c5 demands a yield order), **DG-16** (the workspace name stubs to `l…` instead of dropping — build defect; **falsifies DG-11's "does not reproduce" note**), **DG-17** (c2 + c1 + c4's exemplar copy **cannot coexist** in a 360.66px row — the RULE is wrong, superseded by a graduated copy ladder), plus deferred **DG-18** (the chip's 6px card growth). All routed to `aof:continue 38/04`. **Task 04's `@manual` soak is owed again and was NOT run** — the live control node still runs the `12:14` SEA, which predates this fix, so a soak now would re-soak the broken build. `aof work validate 38/04` → **PASS**
