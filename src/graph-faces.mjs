...
function graphifySkillBody() {
  return [
    ...
    "## Build the graph",
    "",
    "Run `aof graph build <folder>` to extract the graph for a folder. Add",
    "`--backend <name>` only when doc/media extraction is wanted; omit it for an",
    "offline, code-only build (no backend, no key, zero egress).",
    "",
    "## Ask the graph",
    "",
